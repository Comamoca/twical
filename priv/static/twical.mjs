...
function insert3(map5, key2, value2) {
...
var createChildElement = (parent, index5, { key: key2, tag, namespace: namespace2 }) => {
...
function month_from_int(month2) {
  if (month2 === 1) {
...
// build/dev/javascript/twical/types.mjs
var Model = class extends CustomType {
  constructor(select_date) {
    super();
    this.select_date = select_date;
  }
};
var CalendarClicked = class extends CustomType {
};
var ClickedCalendarDay = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
// build/dev/javascript/twical/components.mjs
function calendar() {
  return element2(
    "calendar-date",
    toList([
      ...
      attribute2("value", ""),
      attribute2("locale", "ja-JP"),
      on_change((event4) => {
        return new ClickedCalendarDay(event4);
      })
    ]),
    toList([
      svg(
        toList([
          ...
          attribute2("slot", "previous"),
          class$("fill-current size-4"),
          attribute2("aria-label", "Previous")
        ]),
        toList([
          path(
            toList([
              attribute2("d", "M15.75 19.5 8.25 12l7.5-7.5"),
      ...
      svg(
        toList([
          attribute2("viewBox", "0 0 24 24"),
          attribute2("xmlns", "http://www.w3.org/2000/svg"),
          attribute2("slot", "next"),
          class$("fill-current size-4"),
          attribute2("aria-label", "Next")
        ]),
        toList([
          path(
            toList([
              attribute2("d", "m8.25 4.5 7.5 7.5-7.5 7.5"),
              attribute2("fill", "currentColor")
            ])
          )
        ])
      ),
      element2("calendar-month", toList([]), toList([]))
    ])
  );
}
function parse_date_string(date2) {
  let splited = split2(date2, "-");
  if (splited instanceof Empty) {
    return new Error(void 0);
  } else {
    let $ = splited.tail;
    if ($ instanceof Empty) {
              ...
              return try$(
                ...
                (day3) => {
                  return try$(
                    ...
                    (month3) => {
                      return try$(
                        month_from_int(month3),
                        (month4) => {
                          return new Ok(new Date2(year3, month4, day3));
                        }
                      );
                    }
                  );
...
// build/dev/javascript/twical/twical.mjs
var FILEPATH = "src/twical.gleam";
function init(_) {
  return [new Model(""), none()];
}
function update2(_, msg) {
  if (msg instanceof CalendarClicked) {
    return [new Model(""), none()];
  } else {
    let date2 = msg[0];
    return [
      ...
      from(
        (dispatch) => {
          return addEventListener3(
            "change",
            (_2) => {
              return dispatch(new CalendarClicked());
            }
          );
        }
      )
...
function view(model) {
  let _block;
  let $ = parse_date_string(model.select_date);
  if ($ instanceof Ok) {
    let date3 = $[0];
    _block = to_string(date3.day);
  } else {
    _block = "";
  }
  let date2 = _block;
  return div(
    toList([class$("flex items-center justify-center min-h-screen")]),
    toList([
      div(
        toList([class$("flex flex-col items-center space-y-4")]),
        toList([
          p(toList([]), toList([text2("\u9078\u629E\u3055\u308C\u305F\u65E5\u4ED8: " + date2 + "\u65E5")])),
          calendar()
        ])
      )
    ])
